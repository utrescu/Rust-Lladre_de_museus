...
public abstract class LladreDAOSQL implements LladreDAO {
	...
	public abstract void Connecta(String jdbc, String user, String password) throws SQLException;

	private static final String OBTENIRDENUNCIES = 
			"SELECT m.museu_id, m.nom, d.hora "
			+ "FROM Denuncies d "
	...
	private static final String OBTENIRVISITES = 
			"SELECT DISTINCT c.visitant_id, c.nom "
			+ "FROM Visites v "
			+ "INNER JOIN Visitants c ON v.visitant_id = c.visitant_id "
			+ "WHERE museu_id = ? AND hora = ?";
